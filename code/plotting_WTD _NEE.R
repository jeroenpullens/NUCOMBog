...
par(new=T)
plot(out_NPP,type="l",ylim=c(miny,maxy),col=3,xlab="",ylab="")
par(new=T)
plot(out_DECOMP,type="l",ylim=c(miny,maxy),col=4,xlab="months",ylab="gC m-2 month")
par(new=F)
legend("bottomright",legend=c("measured NEE","modeled NEE","modeled NPP","modeled decomposition"),col = c(2,1,3,4),lty = 1)
