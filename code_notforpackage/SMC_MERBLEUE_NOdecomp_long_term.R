...
setup_SMC<-setup(mainDir="/home/jeroen/MERBLEUE_long_term/",climate="Dataset_1939-2013.txt",environment="Env_Mer_Bleue.txt",inival="Inival_Mer_Bleue.txt",start=1939,end=2013,type=c("NEE","WTD"),numFolders=30000,separate = F,startval=721)
...
# proposal<-function(x){
#   rnorm(length(x),mean=x,sd=values*0.1)
# }

test_smc_nodecomp_long_term<-smc_sampler_mod(likelihood = likelihoodParallel,prior =priortest2,clustertype = "SOCK",numCores = 1,initialParticles = par,setup=setup_SMC, iterations =25, resampling = T, proposal = NULL, parallel="external",scaled=F,Logtype="corrected")
