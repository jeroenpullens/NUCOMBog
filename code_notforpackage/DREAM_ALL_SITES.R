...
setup_SMC_ireland<- setupNUCOM(mainDir="/home/jeroen/IRELAND/",climate="CLIM_IRELAND_1766_2013_PREC_SOIL_evap.txt",environment="Env_IRELAND_staticNdep.txt",inival="inival_IRELAND_centuries.txt",start=1900,end=2012,type=c("NEE","WTD"),numFolders=numFolders,separate = F,startval=1237,parallel = F)
...
setup_SMC_canada<- setupNUCOM(mainDir = "/home/jeroen/MERBLEUE_decomp/", climate = "MERBLEUE_RANDOM_soiltemp_1766_2013.txt",environment = "Env_Mer_Bleue.txt",inival = "Inival_Mer_Bleue.txt",start=1766,end=2013,type=c("NEE","WTD"),numFolders=numFolders,separate = F,startval=2797,parallel = F)
...
setup_SMC_italy <- setupNUCOM(mainDir = "/home/jeroen/NUCOM_ITALY/", climate = "clim_1766_2016.txt",environment = "Env_ITALY.txt",inival = "Inival_ITALY.txt",start = 1766,end = 2015,startval = 2953,type = c("NEE","WTD"))
...
source(file = "/home/jeroen/Desktop/nucom/code_notforpackage/test_DREAMzs_17072017_all.R")

priorrange <- data.frame(min=min, max = max, row.names = names)
date<-Sys.Date()

save.image("~/DREAMzs_all_sites_10_4.RData")
