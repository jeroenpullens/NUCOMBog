...
setup_SMC<- setupNUCOM(mainDir = "/home/jeroen/MERBLEUE_long_term_TEST/", climate = "MERBLEUE_RANDOM_soiltemp_1766_2013.txt",environment = "Env_Mer_Bleue_ndep_measured.txt",inival = "Inival_Mer_Bleue.txt",start=1766,end=2013,type=c("NEE","WTD","hetero_resp","NPP"),numFolders=numFolders,separate = F,startval=2797,parallel = F)
...
save.image("~/MERBLEUE_long_term/DREAMzs_1e6i_soiltemp_ndep_measured_1939_1707_WTL.RData")

pdf(paste("marginalplot_scaled_soiltemp_ndep_measured_",date,".pdf",sep=""))
marginalPlot2(out2,bounds = priorrange)
dev.off()

pdf(paste("marginalplot_unscaled_soiltemp_ndep_measured_",date,".pdf",sep=""))
marginalPlot2(out2)
dev.off()

pdf(paste("correlationplot_soiltemp_ndep_measured_",date,".pdf",sep=""),width=200,height = 200)
