#DEoptim<-runnucom_DEoptim(WD="/home/jeroen/test_package/",climate="ClimateLVM.txt",environment="EnvironmentLVM.txt",inival="inivalLVM.txt",start=1800,end=1805,name.data = "NEE_WTD_GPP_MERBLEUE_1999_2013.csv",sd_NEE = 1,sd_WTD = 1,par = NULL,iter = 1,min=0.5,max=1.5)

runnucom_DEoptim<-function(WD,climate,environment,inival,start,end,name.data,sd_NEE,sd_WTD,par=NULL,iter=200,min,max){
  setwd(WD)
  ...
  values<-c(allpar$values,sd_NEE,sd_WTD)
  names<-c(allpar$names,"sd_NEE","sd_WTD")
