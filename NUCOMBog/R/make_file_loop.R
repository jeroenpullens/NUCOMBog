...
make_param_file<-function(WD,par=NULL){
  ...
  if(!is.null(par)){
    ...
    param1<-which(par$names %in% "dec_param1above")
    param2<-which(par$names %in% "dec_param1below")

    all_param1<-which(allpar$names %in% "dec_param1above")
    all_param2<-which(allpar$names %in% "dec_param1below")

#     if(is.null(param1) & is.null(param2)){
#       allpar$values[alluserprovided]<-par$values[paruserprovided]
#     } else {
#       allpar$values[all_param1]<- -par$values[paruserprovided[param1]]
#       allpar$values[all_param2]<- -par$values[paruserprovided[param2]]
#
#
#       allpar$values[alluserprovided]<-par$values[paruserprovided]
#
